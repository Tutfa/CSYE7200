...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  // 7 points
  val r = new Random(n)
  def next: RandomState[T] = JavaRandomState[T](r.nextLong(), g)
  // Hint: Think of the input and output.
  // 5 points
  def get: T = g(n)
  // Hint: This one need function composition.
  // 13 points
  def map[U](f: T => U): RandomState[U] = flatMap(_ => JavaRandomState(n, g andThen f)) // JavaRandomState[U](n, g.andThen(f))
}
