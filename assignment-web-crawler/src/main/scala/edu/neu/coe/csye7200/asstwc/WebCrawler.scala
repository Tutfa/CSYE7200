...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for {n <- ns \\ "a" map(_ \ "@href")} yield createURL(Some(u), n.toString)
    ...
    // 9 points.
    for {sf <- getURLContent(u); z <- MonadOps.asFuture(getLinks(sf))} yield z
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val usfs: Seq[Future[Seq[URL]]] = for{u <- us2} yield wget(u)
    val usefs: Seq[Future[Either[Throwable, Seq[URL]]]] = for{usf <- usfs} yield MonadOps.sequence(usf)
